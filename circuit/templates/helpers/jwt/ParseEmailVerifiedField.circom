...
template ParseEmailVerifiedField(MAX_KV_PAIR_LEN, MAX_NAME_LEN, MAX_VALUE_LEN) {
    ...
    ParseJWTFieldSharedLogic(MAX_KV_PAIR_LEN, MAX_NAME_LEN, MAX_VALUE_LEN)(field, name, value, field_len, name_len, value_index, value_len, colon_index, 0); // `skip_checks` is set to 0


    signal char_before_value <== SelectArrayValue(MAX_KV_PAIR_LEN)(field, value_index-1);
    ...
    signal char_after_value <== SelectArrayValue(MAX_KV_PAIR_LEN)(field, value_index+value_len);
    ...
    signal whitespace_selector_two[MAX_KV_PAIR_LEN] <== ArraySelectorComplex(MAX_KV_PAIR_LEN)(colon_index+1, value_index-1); // There could potentially be quotes around the value, so we don't contstrain the character before value_index to be whitespace
    signal whitespace_selector_three[MAX_KV_PAIR_LEN] <== ArraySelectorComplex(MAX_KV_PAIR_LEN)(value_index+value_len+1, field_len-1); // similarly to before, don't constrain character just after value end
