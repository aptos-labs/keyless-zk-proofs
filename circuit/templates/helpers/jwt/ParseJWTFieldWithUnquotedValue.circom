...
template ParseJWTFieldWithUnquotedValue(MAX_KV_PAIR_LEN, MAX_NAME_LEN, MAX_VALUE_LEN) {
    ...
    signal whitespace_selector_two[MAX_KV_PAIR_LEN] <== ArraySelectorComplex(MAX_KV_PAIR_LEN)(colon_index+1, value_index); // no quote this time, so check whitespace until the value start
    signal whitespace_selector_three[MAX_KV_PAIR_LEN] <== ArraySelectorComplex(MAX_KV_PAIR_LEN)(value_index+value_len, field_len-1); // and directly after the value end

    signal whitespace_checks[MAX_KV_PAIR_LEN];
    for (var i = 0; i < MAX_KV_PAIR_LEN; i++) {
    ...
    checks[0] <== MultiAND(MAX_KV_PAIR_LEN)(whitespace_checks);
    ...
    signal value_checks[MAX_KV_PAIR_LEN];
    for (var i = 0; i < MAX_KV_PAIR_LEN; i++) {
    ...
    checks[1] <== MultiAND(MAX_KV_PAIR_LEN)(value_checks);
