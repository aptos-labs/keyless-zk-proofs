...
pub static CONFIG: Lazy<ProverServiceConfig> = Lazy::new(|| {
    ...
    let config_file_path = utils::read_environment_variable(CONFIG_FILE_PATH_ENVVAR).unwrap_or_else(|| String::from(CONFIG_FILE_NAME));
    info!("Loading prover service config file from path: {}", config_file_path);
    ...
    match serde_yaml::from_str(&config_file_contents) {
        Ok(config) => config,
        Err(error) => panic!("Failed to parse prover service config yaml file: {}! Error: {}", config_file_path, error),
    }
});
...
impl ProverServiceConfig {
    ...
    pub fn witness_gen_binary_file_path(&self) -> String {
        let witness_gen_binary_file_path = self.setup_directory_path()
    ...
    pub fn test_verification_key_file_path(&self) -> String {
        let test_verification_key_file_path = self.setup_directory_path()
    ...
    pub fn witness_gen_js_file_path(&self) -> String {
        let witness_gen_js_file_path = self.setup_directory_path()
    ...
    pub fn witness_gen_wasm_file_path(&self) -> String {
        let witness_gen_wasm_file_path = self.setup_directory_path()
            .join(MAIN_WASM_FILE_NAME);
    ...
    fn circuit_config_file_path(&self) -> String {
        let circuit_config_file_path = self.setup_directory_path()
    ...
    pub fn load_circuit_params(&self) -> CircuitConfig {
        ...
        match serde_yaml::from_str(&circuit_config_yaml) {
            Ok(config) => config,
            Err(error) => panic!("Failed to parse circuit config yaml file: {}! Error: {}", circuit_config_file_path, error),
        }
    }
    ...
    pub fn load_test_verification_key(&self) -> OnChainGroth16VerificationKey {
        ...
        let test_verification_key_json = utils::read_string_from_file_path(&test_verification_key_file_path);

        // Deserialize the json content into a snarkjs key
        let snarkjs_groth16_verification_key = match serde_json::from_str::<SnarkJsGroth16VerificationKey>(&test_verification_key_json) {
            Ok(snarkjs_groth16_verification_key) => snarkjs_groth16_verification_key,
            Err(error) => panic!("Failed to parse test verification key json file: {}! Error: {}", test_verification_key_file_path, error),
        };
        ...
        match snarkjs_groth16_verification_key.try_as_onchain_repr() {
            ...
            Err(error) => panic!("Failed to convert test verification key to on-chain representation! Error: {}", error),
        }
    }
}
