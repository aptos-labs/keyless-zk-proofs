...
use crate::{
    ...
    training_wheels, utils, witness_gen,
};
use anyhow::Result;
...
pub async fn hande_prove_request(
    ...
    let request_input: RequestInput = match serde_json::from_slice(&request_bytes) {
        ...
        Err(error) => {
            ...
            warn!("{}", error_string);
            return utils::generate_bad_request_response(origin, error_string);
        }
    };
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return utils::generate_bad_request_response(
                origin,
    ...
    utils::generate_json_response(origin, StatusCode::OK, response_string)
}
